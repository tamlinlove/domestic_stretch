...
class StretchDexPose:
    ...
    def set_pose(self, pose: list):
        ...
        if len(pose) != 15:
            raise ValueError(
                "StretchDexPose::set_pose: pose must be a list of length 15"
            )
        self.pose = pose
...
class StretchDexIK:
    ...
    def pose_list_to_dict(self, pose: list) -> dict:
        """
        Converts a pose list to a dictionary.
        ...
        stretch_pose.set_pose(pose)
        return stretch_pose.get_pose_dict(joint_lookup=self.joint_lookup)
    ...
    def solve_ik(
        self,
        target_pos: List,
        target_ori: Optional[List[List]] = None,
        ...
        if bool_check_error:
            ...
            if soln_pos_error > eps_pos or soln_ori_error > eps_ori:
                print(
                    f"StretchDexIK::solve: IK failed to converge to target pose. Position error: {soln_pos_error}, Orientation error: {soln_ori_error}"
                )
                return None
