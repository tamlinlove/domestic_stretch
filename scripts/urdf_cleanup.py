...
def make_ik_urdf(save_path: str):
    ...
    urdf_path = str(
        (
            pathlib.Path(hu.get_fleet_directory()) / "exported_urdf" / "stretch.urdf"
        ).absolute()
    )
    ...
    links_to_remove = [
        l for l in modified_urdf._links if l.name in names_of_links_to_remove
    ]
    ...
    joints_to_remove = [
        l for l in modified_urdf._joints if l.name in names_of_joints_to_remove
    ]
    ...
    link_base_translation = urdfpy.Link(
        name="link_base_translation", inertial=None, visuals=None, collisions=None
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_path", type=str, help="Path to save the modified URDF file."
    )
